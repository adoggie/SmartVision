...
public:
	...
	void onCallIn(const std::shared_ptr<MessageCall> & msg,const Connection::Ptr& conn);
	...
	std::shared_ptr<Message> keepCall(const std::string & sid);    // 保持呼叫线路 （由app定时发送)
	void rejectCall();  // 拒接
	std::shared_ptr<Message> acceptCall();  // 接听
//	bool callOut(const CallInfo& info);  // 呼叫对方
	bool callOut(const std::shared_ptr<MessageCall>& message);  // 呼叫对方
//	void sendMessageOnCalling(const Message::Ptr& msg); // 直接往呼叫通道上发送消息

	void setCallInDenied(int delay);	//不允许其它业主呼入，设置室内机不接受其它业主的呼叫， 0 ： 取消 ， 单位: 秒
...
private:
	...
	std::atomic<std::uint64_t >  live_time_inside_ ;		//室内机发送消息时间
	std::atomic<std::uint64_t >  live_time_outside_;	//外部设备进入消息时间
	boost::asio::steady_timer deadline_timer_;

	std::size_t  work_interval_ = 5;				//工作定时器间隔
	boost::asio::steady_timer work_timer_;		// 工作定时器
//	std::shared_ptr<std::thread> thread_;

	std::atomic_int  	callin_denied_delay_  ; //禁止呼入时间 ， 小于等于0 则放行
};
