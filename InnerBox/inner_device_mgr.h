 ...
 *
 * onXXX() 处理室内App的连接事件
 * callIn,endCall 由 CallManager直接调用，处理 对外连接
 *
 */
...
//	void onMessage(const std::shared_ptr<Message>& message,const std::share_ptr<InnerDevice>& device);
	...
	void onCallOut(const std::shared_ptr<MessageCall>  & msg ,const Connection::Ptr& conn);
...
//	void postAlarm(const std::shared_ptr<SensorAlarmInfo> & alarm);     /*!< 广播报警到连接的室内设备 */
	std::shared_ptr<Message> callIn(const std::shared_ptr<CallRequest>& req);     /*!< 呼叫进入 */
	
	void endCall();     // 远端结束呼叫   (app 发送挂断)
	std::shared_ptr<Message> keepCall(const std::string & sid);    // 远端保持呼叫线路 （由app定时发送)
	...
	void postEmergency(const std::shared_ptr<MessageEmergency> & message); //广播报警消息
protected:
...
//	boost::asio::io_service io_service_;
	std::shared_ptr<CallRequest>  call_req_; //呼叫进入，accept/reject/timeout将清除此对象
};
